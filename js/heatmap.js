...
// 🧱 Dynamic Liquidity Wall Lines
let activeLiquidityLines = []; // { price, isAsk, intensity, startTime }
...
function getBinnedDOM(startPrice, endPrice, step) {
    ...
    const process = (data, bins) => {
        Object.keys(data).forEach(pStr => {
            const p = parseFloat(pStr);
            // Relax the boundary a bit to make sure nothing is skipped
            if (p < startPrice - step*5 || p > endPrice + step*5) return;
            const bin = Math.round(p / step) * step;
            bins[bin] = (bins[bin] || 0) + data[pStr];
        });
    };
    
    process(merged.bids, bidBins);
    process(merged.asks, askBins);

    const result = [];
    Object.keys(bidBins).forEach(pStr => {
        result.push({ p: parseFloat(pStr), q: bidBins[pStr], isAsk: false });
    });
    Object.keys(askBins).forEach(pStr => {
        // If rounding causes an overlap, both will be drawn. We want Asks to correctly render as Asks.
        result.push({ p: parseFloat(pStr), q: askBins[pStr], isAsk: true });
    });
...
function getVividColor(intensity, alpha = 1.0, isAsk = true) {
    if (intensity <= 0) return 'rgba(0,0,0,0)';

    // ATAS Style: Sharper transitions
    // Ask (Sell) side: Red/Orange theme
    const ASK_COLORS = [
        { threshold: 0.0,  color: [0,  10,  40] },   // Near-zero: dark navy
        { threshold: 0.2,  color: [120, 20,  60] },  // Low: dark maroon
        { threshold: 0.5,  color: [200, 40,  40] },  // Medium: bright red
        { threshold: 0.8,  color: [255, 120, 0] },   // High: orange
        { threshold: 1.0,  color: [255, 255, 0] }    // Max: yellow heat
    ];

    // Bid (Buy) side: Cyan/Blue theme
    const BID_COLORS = [
        { threshold: 0.0,  color: [0,  10,  40] },   // Near-zero: dark navy
        { threshold: 0.2,  color: [0,  60,  150] },  // Low: deep blue
        { threshold: 0.5,  color: [0,  150, 200] },  // Medium: bright cyan
        { threshold: 0.8,  color: [0,  255, 255] },  // High: electric cyan
        { threshold: 1.0,  color: [255, 255, 255] }  // Max: white heat
    ];

    const colors = isAsk ? ASK_COLORS : BID_COLORS;
    const idx = colors.findIndex(c => c.threshold >= intensity);
    
    if (idx <= -1) {
        const c = colors[colors.length - 1].color;
    ...
    const lower = colors[idx - 1];
    const upper = colors[idx];
...
let isMarketClosedWeekend = false;
let marketCheckInterval = null;

function isGoldMarketOpen() {
    const now = new Date();
    // Safely get New York time parts
    ...
    // Forex weekend closure: Friday 17:00 to Sunday 18:00 (NY Time)
...
function checkMarketStatus() {
    ...
    const currentlyOpen = isGoldMarketOpen();

    if (!currentlyOpen && !isMarketClosedWeekend) {
        console.log('[Heatmap] ⛔ Market Closed. Pausing WebSockets.');
        isMarketClosedWeekend = true;
        updateMarketStatusUI(false);
        closeWebSockets();
    } else if (currentlyOpen && isMarketClosedWeekend) {
        console.log('[Heatmap] 🟢 Market Opened. Resuming WebSockets.');
        isMarketClosedWeekend = false;
        updateMarketStatusUI(true);
        startWebSockets(heatmapPair);
    }
}
...
async function openHeatmap(pair) {
    ...
    if (pairBadge) {
        // Display USD to user even if internal pair is USDT
        const displayPair = pair.replace('USDT', 'USD');
        if (pairBadge.tagName === 'INPUT') pairBadge.value = displayPair;
        else pairBadge.innerText = displayPair;
    }
    ...
    // Check initial state before starting data streams
    isMarketClosedWeekend = !isGoldMarketOpen();
    updateMarketStatusUI(!isMarketClosedWeekend);

    if (!isMarketClosedWeekend) {
        startWebSockets(pair); // Start live data ONLY if market is open
    } else {
        console.log('[Heatmap] ⛔ Initialized during weekend. WebSockets paused.');
    }

    if (marketCheckInterval) clearInterval(marketCheckInterval);
    marketCheckInterval = setInterval(checkMarketStatus, 5000);
...
async function fetchKlines(pair) {
    try {
        const symbol = pair.replace('/', '').toUpperCase();
        // Use regular futures klines — getY() will apply priceOffset so candles & bubbles align
...
function startWebSockets(pair) {
    ...
    const streams = [
        `${symbol}@depth50@100ms`,
        `${symbol}@aggTrade`,
        `${symbol}@markPrice`
    ];
    const wsUrl = `wss://fstream.binance.com/stream?streams=${streams.join('/')}`;
    console.log(`[Heatmap] 🔌 Connecting WS: ${wsUrl}`);
    depthWS = new WebSocket(wsUrl);
    
    depthWS.onopen = () => console.log(`[Heatmap] ✅ WS Connected`);
    depthWS.onerror = (e) => console.error(`[Heatmap] ❌ WS Error:`, e);
    ...
    depthWS.onmessage = (event) => {
        ...
        if (stream.endsWith('@depth50@100ms')) {
            heatSnapshots.push({
            ...
            if (currentPrice > 0) priceOffset = indexPrice - currentPrice;
        }
    ...
    function handleTradeData(data) {
        ...
        if (indexPrice > 0) priceOffset = indexPrice - currentPrice;
        
        const delta = data.m ? -parseFloat(data.q) : parseFloat(data.q);
        const qty = parseFloat(data.q);

        // 🐋 WHALE SOUND TRIGGER
        if (document.getElementById('whaleSound')?.checked && qty * currentPrice > 10000) { 
            initAudio();
            playSynthSound(150, 'triangle', 0.3, 0.1);
        }
        ...
        // Store futures price — getY() applies priceOffset so candles & bubbles align
        const candleTime = Math.floor(Date.now() / 60000) * 60000;
        let lastCandle = klines[klines.length - 1];
        if (!lastCandle || lastCandle.time !== candleTime) {
            klines.push({ time: candleTime, open: currentPrice, high: currentPrice, low: currentPrice, close: currentPrice, delta: delta });
            if (klines.length > MAX_KLINES) klines.shift();
        } else {
            lastCandle.high = Math.max(lastCandle.high, currentPrice);
            lastCandle.low = Math.min(lastCandle.low, currentPrice);
            lastCandle.close = currentPrice;
            lastCandle.delta += delta;
        }
    }

    // Trade WS is now handled in the combined stream above
    // Keeping a placeholder if needed for legacy, or removing it.
    // I'll comment out the tradeWS part to avoid double connection.
...
function calculateMarketIntelligence() {
    ...
    // 1. TAPE SPEED
    const hudTape = document.getElementById('hudTape');
    if (hudTape) {
        if (currentTPS > 25) {
            hudTape.innerText = `${currentTPS.toFixed(1)}/s [EXTREME🔥]`;
            hudTape.style.color = '#ef4444';
        } else if (currentTPS > 8) {
            hudTape.innerText = `${currentTPS.toFixed(1)}/s [FAST⚡]`;
            hudTape.style.color = '#f59e0b';
        } else {
            hudTape.innerText = `${currentTPS.toFixed(1)}/s [NORMAL]`;
            hudTape.style.color = '#10b981';
        }
    }

    // 2. WHALE SENTIMENT
    const hudWhales = document.getElementById('hudWhales');
    if (hudWhales && currentPrice > 0) {
        const now = Date.now();
        // Look at last 5 minutes
        const recentTrades = tradeBubbles.filter(t => t.time > now - 300000);
        let whaleBuy = 0;
        let whaleSell = 0;
        
        recentTrades.forEach(t => {
            // Whale threshold: ~$25,000 notional (e.g. qty * currentPrice > 25000)
            const notionalBuy = t.buyQty * currentPrice;
            const notionalSell = t.sellQty * currentPrice;
            
            if (notionalBuy > 25000) whaleBuy += t.buyQty;
            if (notionalSell > 25000) whaleSell += t.sellQty;
        });

        const totalWhale = whaleBuy + whaleSell;
        if (totalWhale > 0) {
            const buyPct = (whaleBuy / totalWhale) * 100;
            if (buyPct > 60) {
                hudWhales.innerText = `${buyPct.toFixed(0)}% BUYING 🔼`;
                hudWhales.style.color = '#10b981';
            } else if (buyPct < 40) {
                hudWhales.innerText = `${(100 - buyPct).toFixed(0)}% SELLING 🔽`;
                hudWhales.style.color = '#ef4444';
            } else {
                hudWhales.innerText = `NEUTRAL (50/50) ⚪`;
                hudWhales.style.color = '#9ca3af';
            }
        } else {
            hudWhales.innerText = `NO WHALES 💤`;
            hudWhales.style.color = '#6b7280';
        }
    }

    // 3. CVD DIVERGENCE
    const hudCvd = document.getElementById('hudCvd');
    if (hudCvd) {
        if (klines.length > 5 && cvdData.length > 10) {
            const currentK = klines[klines.length - 1];
            const oldK = klines[Math.max(0, klines.length - 5)];
            const priceTrend = currentK.close - oldK.close;

            const currentC = cvdData[cvdData.length - 1];
            const oldC = cvdData[Math.max(0, cvdData.length - 10)];
            const cvdTrend = currentC.value - oldC.value;

            // Normalize to check if they are going in opposite directions
            if (priceTrend > 0 && cvdTrend < 0) {
                hudCvd.innerText = `BEARISH DIVERG ⚠️`;
                hudCvd.style.color = '#ef4444';
            } else if (priceTrend < 0 && cvdTrend > 0) {
                hudCvd.innerText = `BULLISH DIVERG ❇️`;
                hudCvd.style.color = '#10b981';
            } else {
                hudCvd.innerText = `ALIGNED 🟢`;
                hudCvd.style.color = '#10b981';
            }
        } else {
            hudCvd.innerText = `CALCULATING...`;
            hudCvd.style.color = '#6b7280';
        }
    }
}

// --- SMART TRADE DECISION METER ---
function calculateOrderFlowBias() {
    if (!currentPrice || !heatmapActive) return;

    let deltaScore = 0;
    let domScore = 0;

    // 1. Delta (Short term market buying vs selling over last 60 seconds)
    const now = Date.now();
    const recentDeltaTrades = tradeBubbles.filter(t => t.time > now - 60000);
    let buyVol = 0;
    let sellVol = 0;
    recentDeltaTrades.forEach(t => {
        buyVol += t.buyQty;
        sellVol += t.sellQty;
    });
    ...
    if (totalVol > 0) {
        const deltaRatio = (buyVol - sellVol) / totalVol;
        deltaScore = deltaRatio * 100;
    }

    // 2. DOM Imbalance (Limit Order resting pressure)
    // Within 0.25% of current price
    const range = currentPrice * 0.0025;
    let bidSum = 0;
    let askSum = 0;
    
    if (deepDOMData.bids) {
        deepDOMData.bids.forEach(l => { if (l[0] >= currentPrice - range) bidSum += l[1]; });
        deepDOMData.asks.forEach(l => { if (l[0] <= currentPrice + range) askSum += l[1]; });
    }

    const totalDom = bidSum + askSum;
    if (totalDom > 0) {
        const domRatio = (bidSum - askSum) / totalDom;
        domScore = domRatio * 100;
    }

    // Combine: 60% DOM, 40% Delta
    const bias = Math.round((domScore * 0.6) + (deltaScore * 0.4));
    
    // Update UI
    const bar = document.getElementById('flowBiasBar');
    const txt = document.getElementById('flowBiasText');
    if (!bar || !txt) return;

    let fillW = Math.min(100, Math.abs(bias));
    let color = '#ffffff';
    let label = 'NEUTRAL';
    
    if (bias >= 40) {
        color = '#10b981'; label = 'STRONG BUY';
    } else if (bias > 10) {
        color = '#34d399'; label = 'BUY';
    } else if (bias <= -40) {
        color = '#ef4444'; label = 'STRONG SELL';
    } else if (bias < -10) {
        color = '#f87171'; label = 'SELL';
    }

    // Center-out progress bar logic
    if (bias >= 0) {
        bar.style.left = '50%';
        bar.style.right = 'auto';
        bar.style.width = `${fillW / 2}%`;
    } else {
        bar.style.left = 'auto';
        bar.style.right = '50%';
        bar.style.width = `${fillW / 2}%`;
    ...
    txt.innerText = `${label} ${Math.abs(bias)}%`;
}

/**
 * 🧱 Update and maintain price-sensitive liquidity lines.
 * Checks for high-volume walls and removes them if the price crosses them.
 */
function updateActiveLiquidityLines(domLevels, currentP) {
    const now = Date.now();
    // Sync threshold with GUI heat sensitivity
    const threshold = 0.35 + (heatSensitivity * 0.45);
    const maxDOMQty = Math.max(1, ...domLevels.map(l => l.q));
    
    // 1. Detect new walls from current DOM
    domLevels.forEach(l => {
        const intensity = l.q / maxDOMQty;
        if (intensity > threshold) {
            // Check if we already have a line at this price
            const existing = activeLiquidityLines.find(line => Math.abs(line.price - l.p) < tickSize * 0.4);
            if (!existing) {
                // DON'T add a new line if it's already "broken" (too close to price)
                const isBroken = l.isAsk ? (currentP >= l.p) : (currentP <= l.p);
                if (!isBroken) {
                    activeLiquidityLines.push({
                        price: l.p,
                        isAsk: l.isAsk,
                        intensity: intensity,
                        startTime: now
    ...
    // 2. Clear broken lines (Price Crossing Logic)
    // Red (Ask) broken if shifted price goes ABOVE shifted line price
    activeLiquidityLines = activeLiquidityLines.filter(line => {
        const p = currentP + priceOffset;
        const lineP = line.price + priceOffset;
        if (line.isAsk) {
            return p < lineP; // Keep if price is below resistance
        } else {
            return p > lineP; // Keep if price is above support
        }
    });

    // 3. Cap and Filter
    if (activeLiquidityLines.length > 20) {
        activeLiquidityLines.sort((a, b) => b.intensity - a.intensity);
        activeLiquidityLines = activeLiquidityLines.slice(0, 20);
    }
}
...
function drawMainChart() {
    ...
    if (document.getElementById('autoScale')?.checked) {
        if (currentPrice > 0) {
            const adjustedCurrentP = currentPrice + priceOffset;
            // Always center on currentPrice. Use ±1% as default view window.
            let localMin = adjustedCurrentP * 0.995;
            let localMax = adjustedCurrentP * 1.005;
            ...
            for (let i = heatSnapshots.length - 1; i >= snapLimit; i--) {
                ...
                const adjustedS = s.price + priceOffset;
                if (adjustedS > 0 && Math.abs(adjustedS - adjustedCurrentP) < twoPercent) {
                    localMin = Math.min(localMin, adjustedS);
                    localMax = Math.max(localMax, adjustedS);
                }
            }
            ...
            for (let i = tradeBubbles.length - 1; i >= bubbleLimit; i--) {
                ...
                const adjustedB = b.price + priceOffset;
                if (adjustedB > 0 && Math.abs(adjustedB - adjustedCurrentP) < twoPercent) {
                    localMin = Math.min(localMin, adjustedB);
                    localMax = Math.max(localMax, adjustedB);
                }
            }
    ...
    const drawHistoricalHeatmap = () => {
        ...
        const renderLevels = (levels, drawX, sliceW, alpha = 0.7, isAsk = true) => {
            ...
            levels.forEach(l => {
                const price = l[0];
                const qty = l[1];
                if (price + priceOffset < minPrice - tickSize || price + priceOffset > maxPrice + tickSize || qty <= 0) return;
            ...
            binnedSlice.forEach((qty, binPrice) => {
                ...
                const y = getY(binPrice + priceOffset);
                const color = getVividColor(intensity, alpha, isAsk);
                ctx.fillStyle = color;
        ...
        for (let i = 0; i < visibleSnaps.length - 1; i++) {
            ...
            if (clippedW > 0) {
                renderLevels(s1.bids, clippedX, clippedW, 0.7, false); // false = Bid
                renderLevels(s1.asks, clippedX, clippedW, 0.7, true);  // true = Ask
            }
        }
        ...
        if (visibleSnaps.length > 0) {
            ...
            if (nowX > lastX && lastX < chartW) {
                ...
                if (fillW > 0) {
                    ...
                    renderLevels(lastSnap.bids, fillX, fillW, 0.8, false);
                    renderLevels(lastSnap.asks, fillX, fillW, 0.8, true);
                }
            }
        }
    };
    ...
    if (heatToggle && heatToggle.checked) {
        const domLevels = getBinnedDOM(minPrice, maxPrice, tickSize);
        ...
        domLevels.forEach(l => {
            if (l.p + priceOffset < minPrice || l.p + priceOffset > maxPrice) return;
            
            const rawIntensity = l.q / maxDOMQty;
            const intensity = Math.pow(Math.min(1, rawIntensity), 2.2);
            if (intensity < filterThreshold) return;

            const y = getY(l.p + priceOffset);
        ...
        // 🧱 RENDER DYNAMIC LIQUIDITY LINES
        updateActiveLiquidityLines(domLevels, currentPrice);
        
        activeLiquidityLines.forEach(line => {
            const y = getY(line.price + priceOffset);
            if (y < 0 || y > h) return;
            
            // Vibrant colors for the lines
            const color = line.isAsk ? '#ef4444' : '#10b981'; // Bold Red and Green
            ctx.strokeStyle = color;
            ctx.lineWidth = 3;
            ctx.setLineDash([15, 8]); // Longer dash for better visibility
            ...
            // Price Label on Axis
            ctx.fillStyle = color;
            ctx.fillRect(chartW, y - 10, 55, 20);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 11px monospace';
            ctx.textAlign = 'center';
            ctx.fillText((line.price + priceOffset).toFixed(1), chartW + 27, y + 4);
        });
    }
    ...
    calcTrades.forEach(t => {
        ...
        if (x >= 0 && x <= w) {
            ...
            // POC (Visible only)
            const pBin = Math.round((t.price + priceOffset) * 10) / 10;
    ...
    if (document.getElementById('showImbalance')?.checked) {
        tradeBubbles.forEach(t => {
            ...
            if (ratio > 4 && (t.buyQty + t.sellQty) > maxBinVol * 0.1) { // 4x Imbalance + significant volume
                const y = getY(t.price + priceOffset);
    ...
    if (absorptionZones.length > 0) {
        absorptionZones.forEach(zone => {
            const y = getY(zone.price + priceOffset);
            if (y < 0 || y > h) return;
            ...
            ctx.textAlign = 'right';
            ctx.fillText(`${label} ${zone.strength.toFixed(1)}x`, chartW - 10, y + 4);
    ...
    */
    ...
    if (document.getElementById('showCandles')?.checked && klines.length > 0) {
        ...
        klines.forEach(k => {
            ...
            const yo = getY(k.open + priceOffset);
            const yh = getY(k.high + priceOffset);
            const yl = getY(k.low + priceOffset);
            const yc = getY(k.close + priceOffset);
